...
console.print(f"✅ Logged in as [bold cyan]{identity['Arn']}[/bold cyan]")
        ...
        console.print("[yellow]AWS credentials not found.
...
def select_aws_region(session):
    """Prompt 
user to select an AWS region or enter one manually."""
    try:
        ...
        choices.extend([questionary.Separator(), questionary.Choice(title="[ Manually Enter a Region ]", 
value="__manual__")])
...
def display_asgs(asgs):
    ...
    if not asgs:
        ...
        return
    table = Table(title="Auto Scaling 
Groups", style="cyan", title_style="bold magenta", header_style="bold blue")
...
def create_ec2_instance(ec2_client):
 ...
 console.print(Panel("[bold]Create New EC2 Instance[/bold]", expand=False, border_style="green"))
    try:
        ...
        instance_type = questionary.select("Select an 
instance type:", choices=instance_types, default="t2.micro").ask()
...
vpcs: return
        ...
        subnet_id = questionary.select("Select a Subnet:", 
choices=subnet_choices).ask()
...
[bold]Tags:[/] {', '.join([f"{t['Key']}={t['Value']}" for t in tags])}
      ...
      console.print("[yellow]Instance launch cancelled.[/yellow]")
        ...
        console.print(f"✅ Instance launch request submitted.
ID: [bold cyan]{instance_id}[/bold cyan]")
...
Returns True if terminated."""
    ...
    if action == "terminate" and not questionary.confirm(f"Are you sure you want to terminate {instance_id}?", default=False).ask():
        ...
        return False
    try:
        with console.status(f"[bold 
yellow]Performing '{action}' on {instance_id}...[/bold yellow]"):
            if action == 'start':
...
def modify_termination_protection(ec2_client, instance_id):
  ...
  return
        ...
        console.print(f"✅ [bold green]Termination protection for {instance_id} has been 
set to {'Enabled' if enable else 'Disabled'}.[/bold green]")
        questionary.press_any_key_to_continue().ask()
...
def view_asg_instances(asg_client, ec2_client, asg_name):
    """View the instances attached to a specific Auto Scaling 
Group."""
...
def manage_single_asg(asg_client, ec2_client):
    ...
    asg_choices = [questionary.Choice(title=f"{asg['AutoScalingGroupName']} 
({asg['DesiredCapacity']} instances)", value=asg['AutoScalingGroupName']) for asg in asgs]
...
def update_asg_size(asg_client, asg_name):
    ...
    if max_str is None: return
        ...
        if not (new_min <= new_desired <= new_max):
            console.print("[bold red]Validation Error: Desired capacity must be between min and 
max size.[/bold red]")
            return
...
Please enter numbers only.[/bold red]")
    except IndexError:
        console.print(f"[bold red]Could not find ASG: {asg_name}[/bold red]")
    except ClientError as e:
...
def perform_asg_refresh(asg_client, asg_name):
    ...
    skip_matching = questionary.confirm("Enable 'Skip Matching'? (This 
will ignore instances that don't use the latest launch template)", default=False).ask()
...
refresh_id = response['InstanceRefreshId']
    ...
    status, percentage, status_reason = refresh_details['Status'], refresh_details.get('PercentageComplete', 0), refresh_details.get('StatusReason', '')
                ...
                if status in ['Successful', 'Failed', 'Cancelled', 'Cancelling']:
                    final_status = status
                    if 
status == 'Successful': progress.update(task, completed=100)
                    break
                time.sleep(10)
